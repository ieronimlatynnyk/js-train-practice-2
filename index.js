...
function calculateExpression(expression) {
  // Перевіряємо, чи аргумент є рядком
  if (typeof expression !== "string") {
    return "Argument is not a string";
  }

  try {
    // Виконуємо вираз, використовуючи eval()
    const result = eval(expression);

    // Повертаємо результат обчислення
    return result;
  } catch (error) {
    // В разі виникнення помилки повертаємо повідомлення про помилку
...
function getNumberFromString(s) {
  // Використовуємо parseFloat для того, щоб спробувати отримати число з рядка
  const number = parseFloat(s);

  // Перевіряємо, чи отримане значення є числом та не є NaN
  if (!isNaN(number)) {
    return number; // Повертаємо число, якщо вдалося отримати
  } else {
    return "Couldn't extract a number from the provided string."; // Повертаємо повідомлення про помилку, якщо не вдалося отримати число
  }
}
...
function getIntegerFromString(s) {
  // Використовуємо parseInt для того, щоб спробувати отримати ціле число з рядка
  const integer = parseInt(s);

  // Перевіряємо, чи отримане значення є числом та не є NaN
  if (!isNaN(integer) && Number.isInteger(integer)) {
    return integer; // Повертаємо ціле число, якщо вдалося отримати
  } else {
    return "Couldn't extract an integer from the provided string."; // Повертаємо повідомлення про помилку, якщо не вдалося отримати ціле число
  }
}
